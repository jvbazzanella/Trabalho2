...
Heap::Heap(int MAX_ITEMS){
  ...
  cadastro  = new Veiculo*[MAX_ITEMS];
...
void Heap::enqueue(Veiculo *newItem){
    if (isFull()){
...
void Heap::enqueue(Carro *newItem){
    if (isFull()){
        cout << "heap cheia, remova itens" << endl;
    }
    else {
...
void Heap::enqueue(Barco *newItem){
    if (isFull()){
        cout << "heap cheia, remova itens" << endl;
    }
    else {
...
Veiculo* Heap::dequeue(){
    if (isEmpty()){
        cout << "heap vazia, insira itens" << endl;
        return NULL;
    }
    else{
        Veiculo* temp = cadastro[0];
        this->length--;
